...
function PricingClientInner({ allPlans, currentTeam }: { allPlans: Plan[]; currentTeam?: TeamData }) {
  ...
  const filteredPlans = useMemo(
    () => allPlans.filter((plan) => plan.interval === billingCycle),
  ...
  const handlePlanSelection = async (plan: Plan) => {
    if (plan.amount === 0) {
      ...
      if (result && 'success' in result && !result.success) {
        toast.error(result.error || 'Unable to start checkout for this plan.');
      }
      setLoadingId(null);
    }
  };
  ...
  const confirmFreePlan = async () => {
    ...
    const result = await joinFreePlanAction(formData);
    if (result && 'success' in result && !result.success) {
      toast.error(result.error || 'Unable to switch to the free plan.');
      setLoadingId(null);
      return;
    }
  };
...
export const PricingClient = memo(PricingClientInner);
