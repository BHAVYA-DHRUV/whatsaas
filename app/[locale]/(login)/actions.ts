...
import { getUser, getUserWithTeam, ensureDefaultTeamForUser } from '@/lib/db/queries';
import {
  validatedAction,
...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
  ...
  const authLimits = RATE_LIMITS?.auth ?? {
    requests: 10,
    window: 60_000
  };
  const rl = await rateLimit(`auth:sign-in:${ip}`, authLimits);
  if (!rl.success) {
  ...
  if (!foundTeam) {
    foundTeam = await ensureDefaultTeamForUser(foundUser.id, foundUser.email);
  }
  ...
  redirect(foundTeam?.onboardingCompletedAt ? '/dashboard' : '/onboarding');
});
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  const rl = await rateLimit(`auth:sign-up:${ip}`, authLimits);
  if (!rl.success) {
    return { error: 'Too many attempts. Please try again later.', email, password };
  }
...
//   updateAccountSchema,
//   async (data, _, user) => {
//     const { name, email, enableSignature } =
//       data;

//     const userWithTeam =
//       await getUserWithTeam(user.id);
