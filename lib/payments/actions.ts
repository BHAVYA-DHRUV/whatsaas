...
export const checkoutAction = withTeam(async (formData, team) => {
  ...
  if (plan.amount <= 0) {
    return {
      success: false,
      error: 'Please choose a paid plan or use the free plan option.',
    } satisfies PaymentActionResult;
  }
  ...
  if (plan.gatewayId) {
    ...
    if (!adapter) {
      return {
        success: false,
        error: 'Payment gateway is not configured for this plan.',
      } satisfies PaymentActionResult;
    }
    ...
    return {
      success: false,
      error: 'Checkout session could not be created for this plan.',
    } satisfies PaymentActionResult;
  } else {
    
    const { createCheckoutSession } = await import('./stripe');
    const priceId = plan.gatewayPriceId || plan.stripePriceId;
    if (!priceId) {
      return {
        success: false,
        error: 'Pricing configuration unavailable for this plan.'
      } satisfies PaymentActionResult;
    }
...
export const joinFreePlanAction = withTeam(async (formData, team) => {
  ...
  if (!plan || plan.amount > 0) {
    return {
      success: false,
      error: 'This plan is not available as a free tier.',
    } satisfies PaymentActionResult;
  }
