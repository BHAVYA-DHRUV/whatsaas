...
export const chats = pgTable(
  ...
  (self) => ({
    ...
    teamIdIdx: index('chats_team_id_idx').on(self.teamId),
    teamLastMessageIdx: index('chats_team_id_last_message_timestamp_idx').on(
      self.teamId,
...
// ─── Relations ────────────────────────────────────────────────────────────────
// All relations are defined AFTER every table to avoid undefined references.
...
// FIX 2: teamsRelations moved here — after teamPhoneNumbers, callCredits,
// callLogs, and callCreditTransactions are all declared above.
